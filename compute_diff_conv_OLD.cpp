...
void compute_diffusive_convective(vector<vector<vector<node>>> &mesh, double gamma, string scheme, string type) // type of problem)
{
    ...
    fill_values_at_t(last_value, initial_phi);
    ...
    for (int t = 1; t < time_steps; t++)
    {
        ...
        // initial estimated values: are equal to the previous timestep
        for (int i = 0; i < N; i++)
        {
            for (int j = 0; j < M; j++)
            {
                last_value[i][j] = mesh[t - 1][i][j].phi; // copy phi from previous time step
            }
        }
            ...
            for (int i = 0; i < N; i++)
            {
                for (int j = 0; j < M; j++)
                {
                    ...
                    else if (type == "diagonal")
                    {
                        ...
                        if (i == 0) // Left BC
                        {
                            next_value[i][j] = phi_l;
                        }
                        else if (i == N - 1) // Right BC
                        {
                            next_value[i][j] = phi_h;
                        }
                        else if (j == M - 1) // Top BC
                        {
                            next_value[i][j] = phi_l;
